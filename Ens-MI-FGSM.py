# coding=utf-8
"""Implementation of Ens-MI-FGSM Attack."""
...
tf.flags.DEFINE_string('output_dir', './results/Ens-MI-FGSM/',
        ...
        with tf.Session(config=config) as sess:
            ...
            for filenames, images in load_images(FLAGS.input_dir, batch_shape):
                ...
                for i in range(num_iter):
                    ...
                    noise = sess.run(grad_ensemble, feed_dict={x_input: x, y_input: labels})    
                    noise = noise / np.mean(np.abs(noise), (1, 2, 3), keepdims=True)
                    noise = momentum * grad + noise
                    x = x + alpha * np.sign(noise)
