...
tf.flags.DEFINE_string('output_dir', './results/SVRE-Admix-TI-DIM/',
...
def admix(x,choose_input):
    indices = tf.range(start=0, limit=8, dtype=tf.int32)
    return tf.concat([(x + FLAGS.portion * tf.gather(choose_input, [tf.random.shuffle(indices)[0]])) for _ in range(FLAGS.size)], axis=0)
...
def ensemble_graph(x, one_hot, choose_input,num_classes = 1001):
    x_nes = admix(x,choose_input)
    ...
    noise = tf.reduce_sum(tf.split(noise, FLAGS.size), axis=0)
    return noise
...
def single_graph_inc_v3(x, one_hot,choose_input, num_classes = 1001):
    x_nes = admix(x,choose_input)
    x_batch = tf.concat([x_nes, x_nes/2., x_nes/4., x_nes/8., x_nes/16.], axis=0)
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_inc_v3, end_points_inc_v3 = inception_v3.inception_v3(
...
def single_graph_inc_v4(x, one_hot,choose_input, num_classes = 1001):
    x_nes = admix(x,choose_input)
    x_batch = tf.concat([x_nes, x_nes/2., x_nes/4., x_nes/8., x_nes/16.], axis=0)
    with slim.arg_scope(inception_v4.inception_v4_arg_scope()):
        logits_inc_v4, end_points_inc_v4 = inception_v4.inception_v4(
...
def single_graph_inc_res(x, one_hot,choose_input, num_classes = 1001):
    x_nes = admix(x,choose_input)
    x_batch = tf.concat([x_nes, x_nes/2., x_nes/4., x_nes/8., x_nes/16.], axis=0)
    with slim.arg_scope(inception_resnet_v2.inception_resnet_v2_arg_scope()):
        logits_inc_res, end_points_inc_res = inception_resnet_v2.inception_resnet_v2(
...
def single_graph_res_v2(x, one_hot,choose_input, num_classes = 1001):
    x_nes = admix(x,choose_input)
    x_batch = tf.concat([x_nes, x_nes/2., x_nes/4., x_nes/8., x_nes/16.], axis=0)
    with slim.arg_scope(resnet_v2.resnet_arg_scope()):
        logits_res_v2, end_points_res_v2 = resnet_v2.resnet_v2_101(
...
def main(_):
    ...
    batch_shape_admix = [8, FLAGS.image_height, FLAGS.image_width, 3]
    ...
    with tf.Graph().as_default():
        ...
        one_hot = tf.concat([tf.one_hot(y_input, num_classes)] * 5 * FLAGS.size, axis=0)
        choose_input = tf.placeholder(tf.float32, shape=batch_shape_admix)

        grad_ensemble = ensemble_graph(x_input, one_hot,choose_input)
        grad_inc_v3 = single_graph_inc_v3(x_input, one_hot,choose_input)
        grad_inc_v4 = single_graph_inc_v4(x_input, one_hot,choose_input)
        grad_inc_res = single_graph_inc_res(x_input, one_hot,choose_input)
        grad_res_v2 = single_graph_res_v2(x_input, one_hot,choose_input)
        ...
        with tf.Session(config=config) as sess:
            ...
            idx = 0
            l2_diff = 0
            for filenames, images in load_images(FLAGS.input_dir, batch_shape_admix):
                idx = idx + 1
                print("start the i={} attack".format(idx))

                for index in range(8):
                    labels = []
                    labels.append(f2l[filenames[index]])

                    x = np.copy([images[index]])
                    ...
                    for i in range(num_iter):
                        ...
                        noise_ensemble = sess.run(grad_ensemble, feed_dict={x_input: x, y_input: labels,choose_input:images})
                        ...
                        for j in range(m_svrg):
                            ...
                            noise_x = sess.run(grad_single, feed_dict={x_input: x, y_input: labels,choose_input:images})
                            noise_x_inner = sess.run(grad_single, feed_dict={x_input: x_inner, y_input: labels,choose_input:images})
                    ...
                    save_images(x, [filenames[index]], FLAGS.output_dir)
