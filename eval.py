# coding=utf-8
"""Evaluate the attack success rate under 7 models including normal training models and adversarial training models"""
...
tf.flags.DEFINE_string('eval_file', './results/Ens-I-FGSM/',
                       'eval file.')
...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for model in ['./']:
                import os
                for method in [FLAGS.eval_file]:
                    success_count = np.zeros(len(model_name))
                    input_dir = os.path.join(model, method)
                    print(input_dir)
                    idx = 0
                    for filenames, images in load_images(input_dir, batch_shape):
                        ...
                        for filename, l1, l2, l3, l4, l5, l6, l7 in zip(filenames, v3, v4,res_v2,resnet,ens3_adv_v3,ens4_adv_v3, ens_adv_res_v2):
                            label = f2l[filename]
                    ...
                    s = "|    {:15s}     |    {:10s}    |".format(model, method)
                    for i in range(len(model_name)):
                        print("Attack Success Rate for {0} : {1:.1f}%".format(model_name[i], success_count[i] / 1000. * 100))
                        s += "      {:4.1f}     |".format(success_count[i] / 1000. * 100)
                    print(s)
                    # f = open("Eval_Result.md", "a")
                    # f.write(s + "\n")
                    # f.close()
